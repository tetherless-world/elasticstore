...
class ConfigTest(unittest.TestCase):
    ...
    def setUp(self):
        temp = plugin.get("Elasticsearch", Store)
        print("temp="+str(temp))
    ...
    def test_success(self):
        with patch('elasticstore') as p:
            #self.graph.open({'url': 'sqlite://', 'random_key': 'something'}, create=True)
            self.graph.open({'url': 'http://', 'random_key': 'something'}, create=True)
            p.create_engine.assert_called_with('http://', random_key='something')

    def test_no_url(self):
        # with patch('rdflib_sqlalchemy.store.sqlalchemy'):
        with patch('elasticstore'):
